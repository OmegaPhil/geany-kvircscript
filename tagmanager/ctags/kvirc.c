	...
	vString *const parent, int is_class_parent, const char *arglist)
{
	...
	tag.kindName = KVIrcKinds[K_FUNCTION].name;
	tag.kind = KVIrcKinds[K_FUNCTION].letter;
	...
	if (vStringLength (parent) > 0)
	{
		if (is_class_parent)
		{
			tag.kindName = KVIrcKinds[K_METHOD].name;
			tag.kind = KVIrcKinds[K_METHOD].letter;
	...
	vString *const parent, int is_class_parent)
{
	...
	tag.kindName = KVIrcKinds[K_CLASS].name;
	tag.kind = KVIrcKinds[K_CLASS].letter;
	if (vStringLength (parent) > 0)
	{
		if (is_class_parent)
		{
...
static void makeVariableTag (vString *const var, vString *const parent)
{
	...
	tag.kindName = KVIrcKinds[K_VARIABLE].name;
	tag.kind = KVIrcKinds[K_VARIABLE].letter;
	if (vStringLength (parent) > 0)
	{
		tag.extensionFields.scope [0] = "class";
		tag.extensionFields.scope [1] = vStringValue (parent);
...
static void parseImports (const char *cp)
{
	...
	while (*cp)
	{
		...
		{
			makeSimpleTag (name, KVIrcKinds, K_IMPORT);
		}
	}
	vStringDelete (name);
